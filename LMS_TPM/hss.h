...
/*
 * This is the routine to recover aux data. It generates the aux data in the same way 
 * as the aux data is generated in hss_generate_private_key().
 */
bool hss_recover_aux_data(
    unsigned levels,
    const param_set_t *lm_type,
    const param_set_t *lm_ots_type,
    bool (*read_private_key)(unsigned char *private_key,
            size_t len_private_key, const int autoreserve, void *context),
        void *context,
    size_t len_public_key,
    unsigned char *aux_data, size_t len_aux_data,
    unsigned int autoreserve,
    struct hss_extra_info *info);
...
struct hss_working_key *hss_load_private_key(
    ...
    struct hss_extra_info *info, const int autoreserve);
...
bool hss_get_parameter_set( unsigned *levels,
                           ...
                           bool (*read_private_key)(unsigned char *private_key,
                                       ...
                                       const int autoreserve, void *context),
                           void *context, const int autoreserve);
