...
bool hss_get_parameter_set( unsigned *levels,
                           ...
                           bool (*read_private_key)(unsigned char *private_key,
                                       size_t len_private_key, const int autoreserve,
                                       void *context),
    ...
    if (read_private_key) {
        if (!read_private_key( private_key, PRIVATE_KEY_SEED, autoreserve, context )) {
            goto failed;
        }
    } else {
    ...
    for (level=0; level < MAX_HSS_LEVELS; level++) {
        ...
        switch (lm) {
        case LMS_SHA256_N32_H4:  total_height += 4; break;
