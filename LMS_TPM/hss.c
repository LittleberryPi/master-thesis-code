...
struct hss_working_key *hss_load_private_key(
    ...
    if (!hss_get_parameter_set( &levels, lm, ots, read_private_key, context, autoreserve)) {
    ...
    /* Set the autoreserve count, needed for read_private_key() in hss_generate_working_key()  */
    if (!hss_set_autoreserve(w, autoreserve, NULL)) {
