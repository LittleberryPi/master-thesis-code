...
bool hss_recover_aux_data(
    ...
    unsigned char private_key[ PRIVATE_KEY_LEN ];
    read_private_key(private_key, hss_get_private_key_len(levels, lm_type, lm_ots_type),
                        autoreserve, context);
    ...
    unsigned char seed[SEED_LEN];
    unsigned char I[I_LEN];
    memcpy(seed, private_key+PRIVATE_KEY_SEED, SEED_LEN);
    memcpy(I, private_key+PRIVATE_KEY_SEED+SEED_LEN, I_LEN);
