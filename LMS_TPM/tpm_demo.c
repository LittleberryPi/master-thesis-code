...
const char *default_parm_set = "4/8,4/8,4/8";
...
void int_to_char_array(uint64_t number, unsigned char *buffer);
...
// For TPM
#define TPM_ALWAYS_RESERVE
#define NVINDEX_HBSS_INDEX 0x01000000
#define NVINDEX_PARAM_SEED_I 0x01000001
#define NVINDEX_COUNTER_DIFF 0x01000002
#define NVPWD	"pwd"
...
/*
 * Convert an uint64_t to an unsigned char array of 8
*/
void int_to_char_array(uint64_t number, unsigned char* buffer){
    buffer[0] = number >> 56;
    buffer[1] = number >> 48;
...
/*
 *  This is a function to generate truly random values.
 */
#include "hash.h"
...
bool do_rand( void *output, size_t len ) {
    ...
    struct {
        unsigned char dev_random_output[SEED_LEN];
        int rand_output[16];
    ...
    #ifdef TPM_STORAGE
        unsigned char *random_output = get_random(SEED_LEN);
        memcpy(buffer.dev_random_output, random_output, SEED_LEN);

        /* Also try to grab some output from rand */
        /* In a real program, we'd want to fail if we don't have enough */
        /* entropy, but hey, this is a demo */
        unsigned char *random_output2 = get_random(16);
        memcpy(buffer.rand_output, random_output2, 16);
    #endif // TPM_STORAGE

    /* If we had more random sources, we'd sample them here */

    unsigned output_buffer[SEED_LEN];
    for (i=0; len>0; i++) {
        buffer.count = i;
...
/*
 * This saves the private key to secure storage during key generation; 
 * in this case, a file on the filesystem and the index, compressed param data and
 * seed to TPM NVRAM.
 * The context pointer we use here is the filename.
 */
static bool store_private_key( unsigned char *private_key,
                               __attribute__((unused)) size_t len_private_key, 
                               __attribute__((unused)) void *filename) {
        ...
        if (rc == 0) {
            ...
            if (rc != 0) {
                ...
                rc = defineSpace(tssContext, PRIVATE_KEY_INDEX_LEN, TPMA_NVA_COUNTER, NVINDEX_HBSS_INDEX);
                /* Increment the NV counter */
                if (rc == 0) {
                    if (tpmVerbose) printf("TPM_INFO: Increment the NV counter\n");
                ...
                if (rc == 0) {
                    ...
                    readBuffer = malloc(PRIVATE_KEY_INDEX_LEN);
                    rc = nvRead(tssContext, sessionHandle, readBuffer, NVINDEX_HBSS_INDEX, PRIVATE_KEY_INDEX_LEN, 0);
                }
                ...
                if (rc == 0) {
                    ...
                    if (rc != 0) {
                        if (tpmVerbose) printf("TPM_INFO: Create the NV index at %08x\n", NVINDEX_COUNTER_DIFF);
                        rc = defineSpace(tssContext, PRIVATE_KEY_INDEX_LEN, TPMA_NVA_ORDINARY, NVINDEX_COUNTER_DIFF);
                    }
                }
                /* Write to the NV memory index */
                if (rc == 0) {
                    if (tpmVerbose) printf("TPM_INFO: Write to index %08x and written bit\n", NVINDEX_COUNTER_DIFF);
                    rc = nvWrite(tssContext, sessionHandle, readBuffer, PRIVATE_KEY_INDEX_LEN, NVINDEX_COUNTER_DIFF);
                }
            }
        }
        ...
        /* Store compressed parameter data, seed and I value in a TPM NV memory ordinary index
        Probe to see if the index already exists. */
        if (rc == 0) {
            if (tpmVerbose) printf("TPM_INFO: Read the NV index at %08x\n", NVINDEX_PARAM_SEED_I);
            rc = nvReadPublic(tssContext, NVINDEX_PARAM_SEED_I);
            /* on failure, define the index */
            if (rc != 0) {
                if (tpmVerbose) printf("TPM_INFO: Create the NV index at %08x\n", NVINDEX_PARAM_SEED_I);
                rc = defineSpace(tssContext, PRIVATE_KEY_PARAM_SET_LEN + PRIVATE_KEY_SEED_LEN, TPMA_NVA_ORDINARY, NVINDEX_PARAM_SEED_I);
            }
        }
        /* Write to the NV memory index */
        if (rc == 0) {
            if (tpmVerbose) printf("TPM_INFO: Write to index %08x and written bit\n", NVINDEX_PARAM_SEED_I);
            unsigned char data[PRIVATE_KEY_PARAM_SET_LEN + PRIVATE_KEY_SEED_LEN];
            memcpy(data, private_key + PRIVATE_KEY_PARAM_SET, PRIVATE_KEY_PARAM_SET_LEN + PRIVATE_KEY_SEED_LEN);
            rc = nvWrite(tssContext, sessionHandle, data, PRIVATE_KEY_PARAM_SET_LEN + PRIVATE_KEY_SEED_LEN, NVINDEX_PARAM_SEED_I);
        }
...
/*
 * This updates the private key to secure storage; in this case, a file on the
 * filesystem or TPM NV indices.  The context pointer we use here is the filename
 */
static bool update_private_key( __attribute__((unused)) unsigned char *private_key,
        ...
        /* Verify which part of the private key (index or whole key) needs 
        to be updated */
        switch (len_private_key) {
            case PRIVATE_KEY_INDEX_LEN: // Only update the HBSS index
                ...
                if (rc == 0) {
                    if (tpmVerbose) printf("TPM_INFO: Increment index %08x\n", NVINDEX_HBSS_INDEX);
                    rc = nvIncrement(tssContext, sessionHandle, NVINDEX_HBSS_INDEX);
                }
                break;
            case PRIVATE_KEY_LEN: // HBSS keypair has worn out and we delete all NV indices used
                /* Delete the NV indices */
                nvUndefineSpace(tssContext, NVINDEX_HBSS_INDEX);
                nvUndefineSpace(tssContext, NVINDEX_PARAM_SEED_I);
                nvUndefineSpace(tssContext, NVINDEX_COUNTER_DIFF);
                break;
            default:
                printf("Wrong data length given for update_private_key()\n");
                return false;
        }
...
/*
 * This retrieves the private key from secure storage; in this case, the index is
 * stored in a TPM NV counter and the seed in a TPM NV ordinary index and the
 * compressed parameter set on disk
 */
static bool read_private_key( unsigned char *private_key,
                              size_t len_private_key,
                              const int autoreserve,
                              __attribute__((unused)) void *filename) {
        ...
        /* Read in the HBSS index */
        /* Read the HBSS index from TPM NV memory */
        unsigned char *readBuffer = NULL;
        readBuffer = malloc(PRIVATE_KEY_INDEX_LEN);
        rc = nvRead(tssContext, sessionHandle, readBuffer, NVINDEX_HBSS_INDEX, PRIVATE_KEY_INDEX_LEN, 0);
        ...
        readBuffer = malloc(PRIVATE_KEY_INDEX_LEN);
        rc = nvRead(tssContext, sessionHandle, readBuffer, NVINDEX_COUNTER_DIFF, PRIVATE_KEY_INDEX_LEN, 0);
        uint64_t counter_difference = char_array_to_int(readBuffer);
        free(readBuffer);
        unsigned char actual_hbss_index[8];
        uint64_t actual_hbss_index_int = tpm_counter - counter_difference;
        if (autoreserve > 0) {
            // autoreserve+1 because that is the number we actually reserve
            actual_hbss_index_int = actual_hbss_index_int * (autoreserve+1);
        }
        int_to_char_array(actual_hbss_index_int, actual_hbss_index);
        memcpy(private_key, actual_hbss_index, PRIVATE_KEY_INDEX_LEN);

        /* Read the compressed parameter data only from TPM NV memory */
        readBuffer = malloc(PRIVATE_KEY_PARAM_SET_LEN);
        rc = nvRead(tssContext, sessionHandle, readBuffer, NVINDEX_PARAM_SEED_I, PRIVATE_KEY_PARAM_SET_LEN, 0);
        memcpy(private_key + PRIVATE_KEY_PARAM_SET, readBuffer, PRIVATE_KEY_PARAM_SET_LEN);
        free(readBuffer);


        if (len_private_key == PRIVATE_KEY_LEN) { /* We need to read in the whole private key */
            /* Read the seed and I value from TPM NV memory */
            unsigned char *readBufferSeed = NULL;
            readBufferSeed = malloc(PRIVATE_KEY_SEED_LEN);
            rc = nvRead(tssContext, sessionHandle, readBufferSeed, NVINDEX_PARAM_SEED_I, PRIVATE_KEY_SEED_LEN, PRIVATE_KEY_PARAM_SET_LEN);
            memcpy(private_key + PRIVATE_KEY_SEED, readBufferSeed, PRIVATE_KEY_SEED_LEN);
            free(readBufferSeed);
        }
...
static int keygen(const char *keyname, const char *parm_set) {
    ...
    if (!hss_generate_private_key(
        ...
        store_private_key, private_key_filename, /* Routine to write out */
 ...
 * 
 * The autoreserve number is actually:
 * - for hss_reserve_signature(): autoreserve+1, because in hss_advance_count()
 * the newcount = idx_leaf + 1 + autoreserve.
 * - for hss_advance_count(): autoreserve
 */
static int sign(const char *keyname, const char *text_autoreserve, char **files) {
    int private_key_filename_len = strlen(keyname) + sizeof (".prv" ) + 1;
    const int autoreserve = atoi( text_autoreserve );
    char *private_key_filename = malloc(private_key_filename_len);
    ...
    struct hss_working_key *w = hss_load_private_key(
             ...
             0,                          /* Use the defaults for extra info */
             autoreserve);               /* autoreserve for read_private_key */
    if (!w) {
    ...
    if (w->autoreserve > 0) {
        /* Reserve autoreserve signatures manually */
        if (!hss_reserve_signature(w, update_private_key, private_key_filename,
                autoreserve+1, NULL)) {
            hss_free_working_key(w);
 ...
 * posiitons.
 */
static int advance(const char *keyname, const char *text_advance, const char *text_autoreserve) {
...
/*
 * This function implements the 'recover_aux' command; this reads the key pair file,
 *  and attempts to recover the aux data
 */
static int recover_aux(const char *keyname, const char *text_autoreserve) {
    const int autoreserve = atoi( text_autoreserve );
    /* Parse the parameter set */
    int levels;
    param_set_t lm_array[ MAX_HSS_LEVELS ];
    ...
    /* Format the name of private key filename */
    size_t private_key_filename_len = strlen(keyname) + sizeof (".prv" ) + 1;
    char *private_key_filename = malloc(private_key_filename_len);
    if (!private_key_filename) return 0;
    sprintf( private_key_filename, "%s.prv", keyname );
    // printf( "Recovering aux data  for key %s (will take a while)\n",
                                    //    private_key_filename );
    if (!hss_recover_aux_data(
        levels,        /* # of Merkle levels */
        lm_array, ots_array,  /* The LM and OTS parameters */
        read_private_key, private_key_filename, /* Routine to write out */
                                       /* the genearted private key */
        len_public_key,  /* The public key is placed here */
        aux_size > 0 ? aux : 0, aux_len, /* Where to place the aux data */
        autoreserve, /* The reservation value */
        0)) {            /* Use the defaults for extra info */
            free(private_key_filename);
            free(aux);
            return 0;
...
static int parse_parm_set( int *levels, param_set_t *lm_array,
    ...
    for (i=0;; i++) {
        ...
        param_set_t lm;
        switch (h) {
        case 4:  lm = LMS_SHA256_N32_H4;  break;
...
static void list_parameter_set(int levels, const param_set_t *lm_array,
    ...
    for (i=0; i<levels; i++) {
        ...
        int h = 0;
        switch (lm_array[i]) {
            case LMS_SHA256_N32_H4:  h = 4; break;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        * Ignore the error when the TPM is already initialized. */
        ...
        if (rc != 0) {
            // printf("TPM didn't initialize or is already initialized\n");
        }
    #endif //TPM_STORAGE
    

    /* Depending on the user input, do keygen, sign, verify or advance HBSS index */
    ...
    if (0 == strcmp( argv[1], "sign" )) {
        if (argc < 5) {
            printf( "Error: missing keyname, file argument and autoreserve nr\n" );
            usage(argv[0]);
            return 0;
        }
    ...
    if (0 == strcmp( argv[1], "advance" )) {
        ...
        if (!advance( argv[2], argv[3], argv[4] )) {
            printf( "Error advancing\n" );
        }
        return 0;
    }
    if (0 == strcmp( argv[1], "recover_aux" )) {
        if (argc != 4) {
            printf( "Error: missing or too many parameters\n" );
            usage(argv[0]);
            return 0;
        }
        if (!recover_aux( argv[2], argv[3])) {
            printf( "Error advancing\n" );
        }
        return 0;
    }
